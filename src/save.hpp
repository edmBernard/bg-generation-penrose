...
[[nodiscard]] bool saveTiling(const std::string &filename, const std::vector<penrose::PenroseQuadrilateral> &quad, int canvasSize,
                              RGB rgb1, RGB rgb2, RGB background, int threshold = 6) {
...
[[nodiscard]] bool saveTiling(const std::string &filename, const std::vector<penrose::PenroseQuadrilateral> &quads, int canvasSize,
                              RGB rgb1, RGB rgb2, RGB background, int threshold, int step) {
  ...
  for (auto quad : quads) {
    ...
    const auto newQuad = deflateAndMerge(triangles, step);
    const float strokesWidth = std::sqrt(norm(newQuad[0].vertices[0] - newQuad[0].vertices[1])) / 15.0f;
    if (distrib(gen) > threshold) {
      out << to_path(newQuad, Fill{rgb1}, {}, [&](const penrose::PenroseQuadrilateral &tr) { return isSmall(tr.color) ? distrib(gen) > threshold : false; });
      out << to_path(newQuad, Fill{rgb2}, {}, [&](const penrose::PenroseQuadrilateral &tr) { return !isSmall(tr.color) ? distrib(gen) > threshold : false; });
    } else {
